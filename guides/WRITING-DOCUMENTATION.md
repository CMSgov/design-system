...
# 🚨The anatomy of a page - WORK IN PROGRESS

We use several tools to help generate the documentation. Things are a little funky, so this document is meant to help shine some light on what's actually going on, with the goal of helping think through potential areas of improvement/simplification.

At a high level, each documentation page is a JSON object which is rendered by React. The JSON object's shape is that of a [KssSection](http://kss-node.github.io/kss-node/api/master/module-kss.KssSection.html) (with slight modifications): 

- `depth` (**required**, default: `0`): Indicates how many levels a page is nested. If its `depth` is `0`, then it's a top-level page. The site currently limits the amount of nesting to 2 levels. Any page with a `depth` value of `2` or greater is rendered in the body of its parent. The reason for this is that, by default, each [KSS comment block](/CMSgov/design-system/wiki/writing-documentation#kss-documentation) would get rendered as its own page. This is pretty limiting, and so by restricting the nesting depth, we're able to render multiple KSS comment blocks on the same page, giving us more flexibility with how content is displayed on the page.
- `description`: The page's Markdown body parsed as HTML.
- `header` (**required**): The page's title.
- `markup`: A string holding the HTML code snippet.
- `modifiers`: Array of [KssModifer](http://kss-node.github.io/kss-node/api/master/module-kss.KssModifier.html) objects. We use these to generate additional HTML examples, replacing the `{{modifier}}` template tag in `markup` with the modifier's `className` value.
- `referenceURI` (**required**): The page's URI (ie. `components/button`)
- `sections`: An array of child pages. Initially all sections generated by KSS are in a flat array, so we create this array in our Gulp build process, basing the nesting on the KssSection's `reference` value. This saves us from performing this logic on the front-end in something like a [Redux reducer](http://redux.js.org/docs/basics/Reducers.html).
- `source`: An object representing where in the Sass file the KSS comment block, which was used to generate this page, begins.
