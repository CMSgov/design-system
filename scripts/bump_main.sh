...
TEMP_BRANCH="version-bump-$(date '+%m%d%Y')"
...
yarn lerna version --no-push --exact --exact ${EXTRA_OPTS[@]}
...
gh pr create --base main --title "[RELEASE] Version bump" --body "Please review version updates and compare against latest release branch" --reviewer zarahzachs --reviewer pwolfert --reviewer forrestbaer
