...
const RelativeLink: FC<RelativeLinkProps> = ({ children, headingLevel = '2' }) => {
  const HeadingTag = `h${headingLevel}` as const;
  const text = children.toString().toLowerCase();

  const ariaLabel = `${text} permalink`;
  const relativeLink = text.replace(' ', '-');
  const href = `${window.location.pathname}#${relativeLink}`;
...
const ColorContrastGuidelines = () => {
  return (
    <>
      <RelativeLink>Accessibility considerations</RelativeLink>
      <RelativeLink headingLevel="3">Text contrast</RelativeLink>
      <p>
      ...
      <RelativeLink headingLevel="3">Non-text contrast</RelativeLink>
      <p>
      ...
      <RelativeLink headingLevel="3">Easily test your color combinations</RelativeLink>
      <p>
