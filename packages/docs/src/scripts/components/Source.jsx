...
const Source = props => {
  ...
  if (
    ...
    (props.reactComponentPath || (props.source && !props.source.path.match(/\.md$/)))
  ) {
    ...
    const githubBlobUrl = githubUrl(`blob/master/packages/${packageName}/src/${path}`);

    return (
      <a className={props.className} href={githubBlobUrl} title={`View source of ${path}`}>
        View source file
      </a>
    );
  }
