  ...
  <TableBody>
    ...
    <TableRow key="What's New">
      ...
      <TableCell key="Action" headers="action" stackedTitle="What's New description.">
        ...
        <p>Limited usage only for certain cases in Medicare child system</p>
      </TableCell>
    </TableRow>
  </TableBody>
</Table>
...
### Component behavior (New subsection)
...
- Implementing this pattern requires importing at least 2 components from the Design System:
  1. [Button component](https://design.cms.gov/components/button/?theme=core), and
     2.The specific icon you wish to display (Email icon for example). [Get icon component details](https://design.cms.gov/components/icon/?theme=core).
- The Button component should wrap the icon and be set to the ‘ghost’ variation.
- Changing the size of the Button via the size prop will also change the size of the icon. For more control of the icon size [refer to class names from our font size utilities](https://design.cms.gov/components/icon/?theme=core#changing-icon-size).
