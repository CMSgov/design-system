...
After executing this you will be provided with a list of migration modules to utilize which are named descriptively by function. After a configuration module is chosen, your `cwd` will be searched for files that match the pre-defined rules for migration, and a description of the changes to occur will be presented. Finally, the files to be modified will be listed, and after verification by prompt, migration will take place. **Please be sure to only run these scripts in a git repo where changes can be backed out if incorrect.**
