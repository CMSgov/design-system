...
<Alert variation="warn" heading="The CMS Design System v4 release introduces an updated button architecture that is not backward compatible with previous versions of the design system.">
    <p className="ds-c-alert__text">Review the following sections to become familiar with our new buttons and how to update to them.</p>
</Alert>
...
The updated button structure in Sketch will be in the following folder structure:
Note: the **Ω_unused** folder and **Ω_subcomponents** are for categorization purposes and contain elements that are not used within the theme.
