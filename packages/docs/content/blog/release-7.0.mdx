...
<Alert variation="warn" heading="Skip to v8">
  If you’re just now planning to upgrade to v7, consider upgrading [straight to
  v8](/blog/release-8.0) or higher. The `Autocomplete` and `Dropdown` components underwent some API
  changes that didn't stabilize until v8, so skip this version to avoid changing your code twice.
</Alert>
...
<div className="ds-u-text-align--center ds-u-margin-top--3">
  <img
    ...
    alt="Screenshot of new dropdown component, where the dropdown menu is styled and branded instead of the browser default for HTML select elements"
  />
</div>
