...
The design system includes components for the standard HTML form elements: [checkboxes and radio buttons](https://design.cms.gov/components/choice), [text fields](https://design.cms.gov/components/text-field), [dropdowns ](https://design.cms.gov/components/dropdown), fieldsets, labels, and so on.
...
- Ensure there's sufficient spacing between form fields. Your fields shouldn't feel cramped and users shouldn't confuse which labels and hint text belong to which fields. Use the [margin](https://design.cms.gov/utilities/margin) or [padding](https://design.cms.gov/utilities/padding) utility classes, if necessary, to add extra spacing.
- Each field should have a [`<label>`](https://design.cms.gov/components/form-label).

### Validation

- Place inline validation messages within the field's `<label>` element.
- See [validation guidance for the `<label>` component](https://design.cms.gov/components/form-label/#guidance).

### Required and Optional fields

If most of the fields in a form are required, indicate the few that are optional. If most of the fields in a form are optional, indicate the few that are required. When indicating what form fields are either required or optional, always use text. Use the [`requirementLabel` prop on the `FormLabel` component](https://design.cms.gov/components/form-label/#usage).
