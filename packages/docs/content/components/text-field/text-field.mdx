...
- For numeric inputs (e.g., phone numbers, Social Security numbers, credit card numbers), use the `mask` and `inputMode` props to apply proper formatting. This avoids splitting the input into multiple fields, which can lead to unclear or inaccessible labeling for screen reader users.


<Alert heading="Use semantic grouping for related fields">
