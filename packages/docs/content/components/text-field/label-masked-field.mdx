...
## Accessibility
- Use a `<fieldset>` to group thematically related form controls, and provide a `<legend>` to label the group. This helps screen reader users navigate the form more easily.
- Arrange form blocks vertically. This layout is more accessible, especially for users with limited vision who may struggle to scan horizontally.
- Avoid using placeholder text as a substitute for labels. Most browsers render placeholder text with low contrast, making it difficult to read for many users.
- Do not split numeric inputs (like phone numbers, Social Security numbers, or credit card numbers) into multiple text fields. Use a single field instead. When fields are broken into segments, the associated labels are often unclear or meaningless to screen reader users.
