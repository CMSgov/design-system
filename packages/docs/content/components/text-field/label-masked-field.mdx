...
This component formats the user input in the label as the user types to provide visual feedback about the data. The text in the input field itself will not be formatted until the field loses focus, which is when we know the user has finished typing. We do this to provide a quieter and more usable experience for all users.
