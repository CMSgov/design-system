...
The checkbox button component handles rendering labels, and if you’re using ChoiceList for a set of controls, will also wrap them in `<fieldset>` to maintain accessibility, and `<legend>` to provide context.

### General accessibility guidance
- For a single checkbox, donʼt use `<fieldset>` and `<legend>`.
