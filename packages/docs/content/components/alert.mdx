...
- Users who hear this content through assistive technology, such as a screen reader, will need to be alerted when the alert component visually appears, and that can be done using `aria-live`.
  - E.g., NVDA needs a fraction of a second to catch up with changes in the DOM, use a `setTimeout` to delay displaying the alert.
...
- The element referenced by the `aria-describedby`
...
- When a user's actions require dynamically displaying an alert, such as with success or error states, inject the alert inside an already existing `<div>` with `aria-live="polite"` and `aria-relevant="additions removals"` or `role="alert"`. These ARIA attributes ensure screen readers will announce alerts that have been added or updated dynamically.
  ...
  And whn the user performs an action that displays an alert:
