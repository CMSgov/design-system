...
<h2 class="ds-text-heading--lg">On this page</h2>
...
- Users who hear this content through assistive technology, such as a screen reader, will need to be alerted at the right time, and that can be managed with code.
...
- For dynamic alert content that is injected into the page when it changes, use the `aria-live` attribute, set on an empty element available on page load. The value of the `aria-live` attribute is used to update assistive technologies on the degree of importance.
