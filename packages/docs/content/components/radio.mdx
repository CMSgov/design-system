...
A `ChoiceList` component can be used to render a radio button group or [checkbox](/components/checkbox) group. Use the `type` prop to specify the type of field to display.
