...
<SeeStorybookForGuidance tech="wc" storyId={'web-components-dropdown--docs'} />
...
Because Dropdown is now a custom built solution using Adobe Spectrum instead of a native `<select>` element, [Axe flags the use of a hidden input as a false positive](https://github.com/adobe/react-spectrum/wiki/Known-accessibility-false-positives#picker). This is a known issue but not considered a problem.
