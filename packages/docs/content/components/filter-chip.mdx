...
- The filter chip itself does not need a label, as the text content within it is sufficient. However, actions that the filter can take (like remove) do need to be labelled and you should take advantage of the ariaClearLabel prop to do so. That prop will provide additional text for screen reader support.- The chip should receive focus
...
- `Enter`, `Backspace`, `Delete`, and `Spacebar` should remove the tag or otherwise trigger the filter action when it is in focus
