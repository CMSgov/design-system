...
Before this change, hints and error messages were being rendered inside the `<label>`. There was an exception to this when fields had bottom-placed errors, in which case the error messages would be rendered below the field in their own element and linked to the field via `aria-describedby`. Here are some problems we found with that strategy:
...
<div>
  ...
  <div id="text-field--1__hint" class="ds-c-hint">Helpful hint text</div>
  <div id="text-field--1__error" class="ds-c-inline-error">
    <svg ... />
    Example error message
  </div>
