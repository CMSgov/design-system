...
var Dialog = function Dialog(props) {
  var actions = props.actions,
      ...
      onExit = props.onExit,
      title = props.title,
      modalProps = _objectWithoutProperties(props, ['actions', 'children', 'escapeExitDisabled', 'onExit', 'title']);
  ...
  return _react2.default.createElement(
    ...
    actions && _react2.default.createElement(
      'div',
      { className: 'ds-c-dialog__actions' },
      actions
    )
  );
};
...
Dialog.propTypes = {
  ...
  applicationNode: function applicationNode(props, propName, componentName) {
    if (props[propName] && props[propName] instanceof Element === false) {
      return new Error('Invalid prop `' + propName + '` supplied to `' + componentName + '`. Expected a DOM node. You may also be intersted in the getApplicationNode prop');
    }
  },
  /**
